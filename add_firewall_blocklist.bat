...
for /R %%f in ( *.exe *.bat *.com *.cmd *.sh *.ps1 *.sh *.bin *.run *.out *.command *.app *.elf *.jar *.py *.pl *.deb *.rpm *.tar.gz *.tar *.xz *.zip *.dmg *.app *.pkg *.js *.html *.rb *.gem *.java *.class *.jsp *.war *.html *.htm *.css *.scss *.less *.sass *.php *.py *.pyc *.pyo *.pyd *.lua ) do (
    if /i "%%~nxf" neq "add_firewall_blocklist.bat" if /i "%%~nxf" neq "count_and_list_number_blocked.bat" if /i "%%~nxf" neq "BlockedPrograms.txt" if /i "%%~nxf" neq "%blockedProgramsFile%" if /i "%%~nxf" neq "remove_firewall_blocklist.bat" (
        netsh advfirewall firewall add rule name="[FWB.bat] %%f" dir=out program="%%f" action=block >nul
        netsh advfirewall firewall add rule name="[FWB.bat] %%f" dir=in program="%%f" action=block >nul
